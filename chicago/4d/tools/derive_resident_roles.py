...
def migration_table() -> str:
    ...
    w("")
    w("Five research records hold role evidence about people the residents layer carries:")
    w("the newspaper gazetteer compiled from the 1833-1835 Chicago press, the Fergus 1839")
    w("city register's office tables, the Fergus 1843 and Norris 1844 directory crosswalks,")
    w("and — since T-1507 — the adjudicated readings of retrospective PROSE, which is the")
    w("one of the five that is not a structured record and the reason a trade named only in")
    w("a reminiscence used to be carried by nothing at all. Every row each of them offers")
    w("about a resident is listed below under the disposition it received.")
    w("")
...
def self_test() -> int:
    ...
    print(f"self-test: {63 - len(failures)}/63 assertions hold")
